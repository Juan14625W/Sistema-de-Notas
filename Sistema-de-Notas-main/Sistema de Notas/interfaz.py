"""Interfaz gráfica para el Sistema de Notas.

Este módulo define la clase `App` que construye una aplicación Tkinter con:
- pantalla de login
- menú principal
...
class App:
    """Aplicación principal del sistema de notas con ventanas y navegación."""

    def __init__(self):
        """Inicializa la base de datos, la lógica y la ventana principal."""
        self.db = BaseDatos()
        self.logica = Logica()
        self.ventana = tk.Tk()
    ...
    def crear_entry(self, parent, ancho=30):
        """Crea un `Entry` con estilo consistente para los formularios."""
        return tk.Entry(
            parent,
            width=ancho,
            font=FUENTE_NORMAL,
    ...
    def crear_boton(self, parent, texto, comando, ancho=22, color=None):
        """Crea un botón con estilo uniforme y efecto visual al hacer clic."""
        return tk.Button(
            parent,
            text=texto,
            command=comando,
    ...
    def crear_panel(self, parent, pady=20, padx=30):
        """Crea un contenedor de panel con el color de fondo del sistema."""
        frame = tk.Frame(parent, bg=COLOR_PANEL, bd=0, relief="flat")
        frame.pack(pady=pady, padx=padx, fill="both", expand=True)
        return frame

    def pantalla_login(self):
        """Muestra la pantalla de acceso para el correo institucional."""
    ...
    def pantalla_notas(self):
        """Muestra el formulario para ingresar las notas de los tres cortes."""
    ...
    def calcular_y_guardar(self):
        """Valida las notas, calcula la definitiva y guarda el estudiante en la base de datos."""
        campos = {
    ...
    def pantalla_ver_estudiantes(self):
        """Muestra una lista scrollable de los estudiantes guardados."""
        self.limpiar_ventana()
        self.ventana.configure(bg=COLOR_FONDO)
    ...
    def limpiar_ventana(self):
        """Elimina todos los widgets de la ventana para renderizar otra pantalla."""
